...
#!/usr/bin/env pwsh
# .SYNOPSIS
#   MCP testScript v0.1.1
# .EXAMPLE
#   ./Test-Module.ps1 -version 0.1.1
#   Will test the module in ./BuildOutput/MCP/0.1.1/
# .EXAMPLE
#   ./Test-Module.ps1
#   Will test the latest  module version in ./BuildOutput/MCP/
param (
  ...
  [ValidateScript({
      ...
      $b_Path = [IO.Path]::Combine($PSScriptRoot, 'BuildOutput', 'MCP')
...
begin {
  $TestResults = $null
  $BuildOutput = [IO.DirectoryInfo]::New([IO.Path]::Combine($PSScriptRoot, 'BuildOutput', 'MCP'))
  ...
  $BuildOutDir = Resolve-Path $([IO.Path]::Combine($PSScriptRoot, 'BuildOutput', 'MCP', $version)) -ErrorAction Ignore | Get-Item -ErrorAction Ignore
  if (!$BuildOutDir.Exists) { throw [System.IO.DirectoryNotFoundException]::new($BuildOutDir) }
  $manifestFile = [IO.FileInfo]::New([IO.Path]::Combine($BuildOutDir.FullName, "MCP.psd1"))
  ...
  if (!(Test-Path -Path $([IO.Path]::Combine($PSScriptRoot, "MCP.psd1")) -PathType Leaf -ErrorAction Ignore)) { throw [System.IO.FileNotFoundException]::New("Module manifest file Was not Found in '$($BuildOutDir.FullName)'.") }
  $script:fnNames = [System.Collections.Generic.List[string]]::New(); $testFiles = [System.Collections.Generic.List[IO.FileInfo]]::New()
  [void]$testFiles.Add([IO.FileInfo]::New([IO.Path]::Combine("$PSScriptRoot", 'Tests', 'MCP.Integration.Tests.ps1')))
  [void]$testFiles.Add([IO.FileInfo]::New([IO.Path]::Combine("$PSScriptRoot", 'Tests', 'MCP.Features.Tests.ps1')))
  [void]$testFiles.Add([IO.FileInfo]::New([IO.Path]::Combine("$PSScriptRoot", 'Tests', 'MCP.Module.Tests.ps1')))
}
